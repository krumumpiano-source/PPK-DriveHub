...
export async function onRequest(context) {
  ...
  if (path.match(/^\/api\/reports\/vehicle-timeline\/[^/]+$/) && method === 'GET') {
    ...
    if (!vtType || vtType === 'inspection') { const r = await dbAll(env.DB, `SELECT id,inspection_date,expiry_date,result,cost,inspection_center FROM inspection_records WHERE car_id=? ORDER BY inspection_date DESC`, [vtCarId]); r.forEach(x=>events.push({type:'inspection',date:x.inspection_date,data:x})); }
