...
- Name: e2e\driver.spec.mjs >> 1. Authentication — เข้าสู่ระบบและตรวจสอบตัวตน >> GET /api/auth/me — ดูข้อมูลตัวเองได้
- Location: tests\e2e\driver.spec.mjs:230:3
...
> 234 |     expect(r?.data?.role).toBe('driver');
  ...
  328 |       purpose: 'E2E cancel test',
  329 |     }, ctx.driverToken);
  330 |     const cancelId = createRes?.data?.id;
  331 |     if (!cancelId) return test.skip();
  332 | 
  333 |     const r = await apiDelete(`/api/vehicle-requests/${cancelId}`, ctx.driverToken);
  334 |     expect(r?.success).toBe(true);
```