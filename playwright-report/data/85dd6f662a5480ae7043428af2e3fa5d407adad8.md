...
- Name: e2e\fuel.spec.mjs >> 3. บันทึกน้ำมัน — บริบทพนักงานขับรถ (QR, ไม่มี token) >> บันทึกน้ำมัน minimal ด้วย driver_id (QR scan) → สำเร็จ
- Location: tests\e2e\fuel.spec.mjs:274:3
...
```ts
  184 |   test('ไม่ส่ง car_id → error', async () => {
  185 |     const r = await apiPost('/api/fuel/record', {
  186 |       receipt_image: MOCK_RECEIPT,
  187 |       mileage_after: 10000,
  188 |       purpose: 'business',
  189 |       driver_name_manual: 'คนทดสอบ',
  190 |     });
  191 |     expect(r?.success).toBe(false);
