...
- Name: e2e\ui-mobile-flows.spec.mjs >> C1: Login Flow — Mobile >> password input มี type=password (ปกปิดรหัส)
- Location: tests\e2e\ui-mobile-flows.spec.mjs:77:3
...
Locator:  locator('input[type="password"], #password').first()
Expected: visible
Received: hidden
Timeout:  5000ms
...
Call log:
  ...
  - waiting for locator('input[type="password"], #password').first()
    9 × locator resolved to <input required="" type="password" id="loginPassword" autocomplete="current-password" placeholder="กรอกรหัสผ่านของคุณ"/>
