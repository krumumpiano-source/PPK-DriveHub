...
Locator:  locator('button:has-text("จอง"), button:has-text("เพิ่มคิว"), button:has-text("จองคิว"), [data-action="add-queue"]').first()
Expected: visible
Received: hidden
Timeout:  5000ms
...
Call log:
  ...
  - waiting for locator('button:has-text("จอง"), button:has-text("เพิ่มคิว"), button:has-text("จองคิว"), [data-action="add-queue"]').first()
    9 × locator resolved to <button class="btn btn-success" onclick="openCreateModal()">➕ เพิ่มคิว</button>
